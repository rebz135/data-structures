describe('graph', function() {
  ...
  it('should not execute a callback on removed node', function() {
    var connectToFive = function(item) {
      graph.addEdge(item, 5);
    };
    graph.addNode(5);
  ...
  it('hasEdge should return false if from node or to node do not exist', function() {
    graph.addNode(4);
    graph.addNode(5);
    graph.removeNode(5);
    expect(graph.hasEdge(4, 5)).to.equal(false);
  ...
  it('should only add edges to existing nodes', function() {
    graph.addNode(4);
    graph.addNode(5);
    graph.removeNode(5);
    expect(function() {
